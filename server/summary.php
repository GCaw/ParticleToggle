...
<?php
	...
	if (isset($_GET['code']))
	{
		if ($_GET['code'] == $client_key)
		{
			...
			$numDays = 15;
			$start = $start - (60*60*24*($numDays-1));
			$running = false;
			...
			for ($i = 0; $i < $numDays; $i++)
			{
				...
				try
				{
					...
					if ($res){
						...
						$to_display = sprintf("%.02fhrs", $total_hours);
						echo "Total: " . $to_display . " (" . $running_total . ")<br />";
						if ($running)
						{
							echo "<title>" . $to_display . " -  Toggle Summary</title>";
						}
					}
					else
					{
